 ...
 */
	public function uploadDir($dir = null) {
		...
		if (!is_dir($_fullPath))
			throw new UploadifyComponentException(__d('media',"Upload Directory %s does not exist in %s",$dir, $this->uploadPath));
		elseif(!is_writable($_fullPath))
			throw new UploadifyComponentException(__d('media',"Upload Directory %s in %s is not writeable",$dir, $this->uploadPath));
	...
	private function _dispatch($method) {
		...
		try {
			if (!is_callable(array($this,$method))) {
				throw new UploadifyComponentException(__d('media',"Method UploadifyComponent::%s() does not exist",$method));
			} else {
 ...
 */	
	protected function parseUploadFiles() {
		$uploadErrors = array(
	        0=>__d('media',"There is no error, the file uploaded successfully"),
	        1=>__d('media',"The uploaded file exceeds the upload_max_filesize directive in php.ini"),
	        2=>__d('media',"The uploaded file exceeds the MAX_FILE_SIZE directive that was specified in the HTML form"),
	        3=>__d('media',"The uploaded file was only partially uploaded"),
	        4=>__d('media',"No file was uploaded"),
	        6=>__d('media',"Missing a temporary folder")
		);
		
		// Validate the upload
		try {
			if (empty($_FILES)) {
				throw new UploadifyComponentException(__d('media','No upload detected'));
			} elseif (!empty($_FILES) && !isset($_FILES[$this->uploadName])) {
				throw new UploadifyComponentException(__d('media','Upload with invalid uploadName detected'));
			}elseif (!isset($_FILES[$this->uploadName])) {
				throw new UploadifyComponentException(sprintf(__d('media',"No upload found in \$_FILES for %s"),$this->uploadName));
				...
				throw new UploadifyComponentException(__d('media',"Upload failed is_uploaded_file test."));
			} elseif (!isset($_FILES[$this->uploadName]['name'])) {
				throw new UploadifyComponentException(__d('media',"File has no name."));
			}
		} catch(Exception $e) {
		...
		try {
			...
			if (!$fileSize || $fileSize > $this->fileSizeMax) {
				throw new UploadifyComponentException(__d('media',"File exceeds the maximum allowed size"));
			}
	
			if ($fileSize <= 0) {
				throw new UploadifyComponentException(__d('media',"File size outside allowed lower bound"));
			}
		} catch(Exception $e) {
			throw new UploadifyComponentException($e->getMessage());
			return false;
		...
		try {
			...
			if (!empty($this->extension) && !in_array($ext,$this->extensions))
				throw new UploadifyComponentException(__d('media',"File extension %s is not allowed",$ext));
		...
		try {
			...
			if (strlen($fileName) == 0 || strlen($fileName) > $this->fileNameMaxLength)
				throw new UploadifyComponentException(__d('media',"Invalid file name (max %s chars)",$this->fileNameMaxLength) );
 ...
 */	
	...
	protected function processUploadFiles() {
		...
		if (file_exists($targetFilePath)) {
			throw new UploadifyComponentException(__d('media',"File %s already exists in %s",$this->Upload->fileBasename, $this->uploadDir()));
			return false;
		}
		
		if (!move_uploaded_file($this->Upload->tmpName, $targetFilePath)) {
			throw new UploadifyComponentException(__d('media',"Failed to move file %s to %s",$this->Upload->fileBasename, $this->uploadDir()));
			return false;
		}
		
		$this->response(__d('media',"Upload Successful"),UploadifyUpload::STATUS_SUCCESS);
	}
