...
class AttachmentHelper extends AppHelper {
	...
	public $helpers = array('Html', 'Form', 'Media.Thumb');
	...
	public function thumb($attachment, $thumbOptions = array()) {
		return $this->Thumb->image($attachment['path'], $thumbOptions);
	}

	public function thumbLink($attachment, $url = null, $thumbOptions = array(), $linkOptions = array()) {
		$img = $this->thumb($attachment, $thumbOptions);
		switch ($url) {
			case null:
				$url = $attachment['url'];
				break;
		...
		if ($url) {
			...
			return $this->Html->link($img, $url, $linkOptions);
		}
