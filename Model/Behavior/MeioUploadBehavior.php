 ...
 */
	function _uploadFile(&$model, $data = null) {
		...
		foreach ($this->__fields[$model->alias] as $fieldName => $options) {
			...
			list($uploadName,$ext) = $this->_splitFilenameAndExt($data[$model->alias][$fieldName]['name']);
			...
			if ($options['useTable'] == false) {
				...
				$uploadName = ($options['uploadName']) ? $data[$model->alias][$options['uploadName']] : $uploadName;
				//$this->_includeDefaultReplacement($uploadName.'.'.$sub);
				$this->_fixName($model, $fieldName);
				$saveName = $data[$model->alias][$fieldName]['name'];
 ...
 */
	function _splitFilenameAndExt($filename) {
		...
		if (!isset($info['filename'])) {
			$info['filename'] = substr($info['basename'], 0, -1 - count($info['extension'])); // Remove extension and .
		}
